...
class Reliability:
    """Internal class that computes the standard PLS-SEM reliability metrics
    (Cronbach's alpha and Dillon-Goldstein rho_c) plus the first two
    PCA eigenvalues per LV block. Use the method
    :meth:`~.openpls.Plspm.reliability` defined on :class:`~.openpls.Plspm`
    to retrieve the results.

    Renamed from ``Unidimensionality`` in openpls-engine 1.9.0: those
    coefficients measure *reliability*, not *unidimensionality* (a stricter
    psychometric concept evidenced separately, e.g. via the eigenvalue
    ratio). The old name remains available as a deprecation shim in
    :mod:`openpls.unidimensionality`.
    """
    ...
    def summary(self):
        """Compute the reliability summary table for every LV.

        For latent variables whose indicator block contains missing values,
        the computation falls back to listwise deletion (rows with any NaN
        in the block are dropped) with a per-block correction factor.
        Upstream `plspm` skipped the LV entirely in this case (OpenPLS fix #65).
        """
